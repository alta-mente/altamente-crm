...
export function CashFlowChart({ invoices, projects, services, companyHours }: { invoices: Invoice[], projects?: any[], services?: any[], companyHours?: any[] }) {
  ...
  companyHours?.forEach(h => {
    if (h.billed) {
      ...
      if (h.companies?.hourly_rate) {
        const cost = (h.minutes / 60) * h.companies.hourly_rate
        const groupKey = \`\${dStr}-\${h.project_id || 'no-proj'}\`
        
        if (!hourGroups[groupKey]) {
          const clientName = h.companies?.name ? h.companies.name : 'Cliente'
          const projName = h.projects?.name ? \` (\${h.projects.name})\` : ''
          hourGroups[groupKey] = {
            total: 0,
            dateStr: dStr,
      ...
      <div className={styles.cardContent}>
        <div className={styles.cardTop} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span className={styles.cardLabel} style={{ fontSize: '13px', fontWeight: 600 }}>Cash Flow</span>
...
fs.writeFileSync('src/components/dashboard/CashFlowChart.tsx', code)
